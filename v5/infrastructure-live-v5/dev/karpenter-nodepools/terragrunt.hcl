include "root" {
    path = find_in_parent_folders("root.hcl")
    expose = true
}

include "env" {
    path = find_in_parent_folders("env.hcl")
    expose = true
    merge_strategy = "no_merge"
}

terraform {
    source = "../../../infrastructure-modules/karpenter-nodepools"
}

inputs = {
    eks_full_name = dependency.karpenter.outputs.eks_full_name
    eks_cluster_endpoint = dependency.karpenter.outputs.eks_cluster_endpoint
    cluster_ca_data = dependency.karpenter.outputs.cluster_ca_data
    karpenter_enabled = dependency.karpenter.outputs.karpenter_enabled
    karpenter_node_role = dependency.karpenter.outputs.karpenter_node_role
}

dependency "karpenter" {
    config_path = "../karpenter"

    mock_outputs = {
        eks_full_name = "eks-${include.env.locals.env}-${include.root.locals.project_name}-cluster"
        eks_cluster_endpoint = "https://${include.env.locals.env}-${include.root.locals.project_name}-mock-endpoint"
        cluster_ca_data = "" # base64("mock_cluster_ca_data")
        karpenter_enabled = true
        karpenter_node_role = "karpenter-node-role"
    }
}

generate "kubernetes_provider" {
    path = "kubernetes-provider.tf"
    if_exists = "overwrite_terragrunt"
    contents = <<EOF
provider "kubernetes" {
    host = var.eks_cluster_endpoint
    cluster_ca_certificate = base64decode(var.cluster_ca_data)
    exec {
        api_version = "client.authentication.k8s.io/v1beta1"
        args = ["eks", "get-token", "--cluster-name", var.eks_full_name]
        command = "aws"
    }
}
EOF
}