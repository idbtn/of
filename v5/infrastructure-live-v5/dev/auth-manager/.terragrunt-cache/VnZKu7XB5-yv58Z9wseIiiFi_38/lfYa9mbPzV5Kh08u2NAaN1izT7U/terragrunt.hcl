include "root" {
    path = find_in_parent_folders("root.hcl")
    expose = true
}

include "env" {
    path = find_in_parent_folders("env.hcl")
    expose = true
    merge_strategy = "no_merge"
}

terraform {
    source = "../../../infrastructure-modules/auth-manager"

    before_hook "import_aws_auth_into_auth_manager_state" {
        commands = ["plan", "apply"]
        execute = ["bash", "-c", <<EOT
set -euo pipefail
echo "[auth-manager] Checking if aws-auth is in auth-manager state..."
if terraform state list 2>/dev/null | grep -q 'kubernetes_config_map_v1.aws_auth'; then
    echo "[auth-manager] kubernetes_config_map_v1.aws_auth already in state, skipping import."
else
    echo "[auth-manager] Importing existing aws-auth ConfigMap into state..."
    terraform import kubernetes_config_map_v1.aws_auth kube-system/aws-auth
fi
EOT
]
    }
}

inputs = {
    karpenter_aws_auth_role_mapping = dependency.karpenter.outputs.karpenter_aws_auth_role_mapping
    eks_full_name = dependency.eks.outputs.eks_full_name
    cluster_endpoint = dependency.eks.outputs.cluster_endpoint
    cluster_ca_data = dependency.eks.outputs.cluster_ca_data
}

dependency "karpenter" {
    config_path = "../karpenter"

    mock_outputs = {
        karpenter_aws_auth_role_mapping = {
            rolearn = "arn:aws:iam::123456789012:role/eks-dev-of-karpenter-node-role"
            username = "system:node:{{EC2PrivateDNSName}}"
            groups = [
                "system:bootstrappers",
                "system:nodes",
                "system:node-proxier",
            ]
        }
        mock_outputs_allowed_terraform_commands = ["init", "validate"]
    }
}

dependency "eks" {
    config_path = "../eks"

    mock_outputs = {
        eks_full_name = "eks-${include.env.locals.env}-${include.root.locals.project_name}-cluster"
        cluster_endpoint = "https://${include.env.locals.env}-${include.root.locals.project_name}-mock-endpoint"
        cluster_ca_data = "" # base64("mock_cluster_ca_data")
        openid_provider_arn = "arn:aws:iam::123456789012:oidc-provider/oidc.eks.${include.root.locals.aws_region}.amazonaws.com/id/123456789012345"
        openid_provider_url = "https://oidc.eks.${include.root.locals.aws_region}.amazonaws.com/id/123456789012345"
        subnet_ids = ["subnet-1234", "subnet-5678"]
    }
    mock_outputs_allowed_terraform_commands = ["init", "validate"]
}

generate "kubernetes_provider" {
    path = "kubernetes-provider.tf"
    if_exists = "overwrite_terragrunt"
    contents = <<EOF
provider "kubernetes" {
    host = var.cluster_endpoint
    cluster_ca_certificate = base64decode(var.cluster_ca_data)
    exec {
        api_version = "client.authentication.k8s.io/v1beta1"
        args = ["eks", "get-token", "--cluster-name", var.eks_full_name]
        command = "aws"
    }
}
EOF
}